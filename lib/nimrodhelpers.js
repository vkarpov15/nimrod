...
Array.contains = function(arr, content) {
  return arr.indexOf(content) !== -1;
}
...
module.exports = {
  ...
  runCommand: function(conn, obj) {
    return conn.flow.sync(function(callback) {
      conn.db.command(obj, function(error, result) {
        if (error) {
          return callback(null,
            { ok: 0, err: error, errmsg: error.toString() });
        }
  ...
  runAdminCommand: function(conn, obj) {
    return conn.flow.sync(function(callback) {
      conn.db.admin().command(obj, function(error, result) {
        if (error) {
          return callback(null,
            { ok: 0, err: error, errmsg: error.toString() });
        }
