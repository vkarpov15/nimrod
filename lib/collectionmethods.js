...
function ShellIterator(conn, coll) {
  ...
  ret._addSpecial = function(name, value) {
    cursor.cmd[name.replace('$', '')] = value;
    return this;
  };
  ...
  return function(collectionName) {
    return {
      ...
      insert: function(doc, options) {
        ...
        convertFunctionsToBSONCode(doc);
      ...
      group: function(obj) {
        if (!obj.ns) {
          obj.ns = collectionName;
        }
        return this.getDB().group(obj);
      },
      ...
      mapReduce: function(map, reduce, options) {
        var data = flow.sync(function(callback) {
          conn.db.collection(collectionName).
            mapReduce(map, reduce, options, function(error, result) {
              callback(error, result);
            });
        });
        var db = this.getDB();
        return new MapReduceResult(db[data.s.name], db[data.s.name].find({}));
      },
      ...
      save: function(obj, opts) {
        return flow.sync(function(callback) {
          convertFunctionsToBSONCode(obj);
...
function convertFunctionsToBSONCode(doc) {
  for (var key in doc) {
    if (typeof doc[key] === 'function') {
      doc[key] = new Code(doc[key]);
    }
  }
}
