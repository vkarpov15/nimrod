...
function RSHelpers(conn) {
  ...
  this.initiate = function(conf) {
    return helpers.cleanupDocs(
      helpers.runCommand(conn, { "replSetInitiate" : conf })
    );
  };
  ...
  this.freeze = function (secs) {
    return helpers.cleanupDocs(
      helpers.runCommand(conn, { "replSetFreeze" : secs })
    );
  };
  ...
  this.remove = function(hn) {
    ...
    for (var i in c.members) {
      if (c.members[i].host === hn) {
        ...
        return helpers.cleanupDocs(
          helpers.runCommand(conn, { "replSetReconfig": c })
        );
      }
    }
  ...
  this.status = function() {
    return helpers.cleanupDocs(
      helpers.runCommand(conn, {"replSetGetStatus": 1})
    );
  };

  this.stepDown = function (secs) {
    return helpers.cleanupDocs(
      helpers.runCommand(conn, { "replSetStepDown": (secs === undefined) ? 60:secs})
    );
  };

  this.syncFrom = function (host) {
    return helpers.cleanupDocs(
      helpers.runCommand(conn, { "replSetSyncFrom": host })
    );
  };
  ...
  this.add = function (hostport, arb) {
    ...
    return helpers.cleanupDocs(
      helpers.runCommand(conn, { "replSetReconfig": conf})
    );
  };
